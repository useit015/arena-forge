...
class RenderImageDialog {
  constructor(editor, strings) {
    ...
    titleBar.textContent =
      strings.getKey('menubar/render') + ' ' + strings.getKey('menubar/render/image');
    ...
    const shadingTypeSelect = new UISelect()
      .setOptions({
        solid: 'SOLID',
        realistic: 'REALISTIC',
      })
      .setWidth('170px')
      .onChange(refreshShadingRow)
      .setValue('solid');
      ...
      switch (shadingTypeSelect.getValue()) {
        ...
        case 'realistic':
          ...
          const maxSamples = Math.max(
            ...
            Math.min(pathTracerMaxSamples, samplesNumber.getValue())
          );
...
class RenderVideoDialog {
  constructor(editor, strings) {
    ...
    titleBar.className = 'Dialog-title';
    titleBar.textContent =
      strings.getKey('menubar/render') + ' ' + strings.getKey('menubar/render/video');
    content.appendChild(titleBar);
    ...
    const videoQuality = new UISelect()
      .setOptions({
        low: 'Low',
        medium: 'Medium',
        high: 'High',
...
function createMP4(chunks, avcC, width, height, fps) {
  ...
  // stts - Time-to-Sample
  const stts = fullBox('stts', 0, 0, u32(1), u32(chunks.length), u32(frameDuration));
