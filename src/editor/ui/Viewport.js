...
import { EditorControls } from '../core/EditorControls.js';
...
function Viewport (editor) {
    ...
    if (onDownPosition.distanceTo(onUpPosition) === 0) {
      ...
      if (editor.currentTool === 'add') {
        ...
        if (raycaster.ray.intersectPlane(plane, target)) {
          // Snap Logic
          // Assuming basic snap for now if needed, or rely on future Editor state
          // LevelEditor used editor.grid.snapToGrid

          // Get Object Type
          const type = editor.currentObjectType || 'box';

          // Execute Command
          editor.execute(new AddObjectCommand(editor, type, target));

          // Switch back to select?
          editor.signals.toolChanged.dispatch('select');
        }
        return;
      }
  ...
  signals.rendererCreated.add(function (newRenderer) {
    if (renderer !== null) {
      ...
      if (renderer.domElement && container.dom.contains(renderer.domElement)) {
        container.dom.removeChild(renderer.domElement);
      }
    }

    controls.connect(newRenderer.domElement);
    transformControls.domElement = newRenderer.domElement;
        ...
        case 'Default':
          ...
          if (scene.background !== null) {
            if (scene.background.isColor) {
              ...
              colorScene.background = scene.background;
              scene.environment = pmremGenerator.fromScene(colorScene, 0.04).texture;
  ...
  const timer = new Timer(); // only used for animations
