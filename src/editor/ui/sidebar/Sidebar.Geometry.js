...
function SidebarGeometry(editor) {
  ...
  */
  ...
  geometryBoundingBoxRow.add(
    new UIText(strings.getKey('sidebar/geometry/bounds')).setClass('Label')
  );
  ...
  const geometryUserData = new UITextArea()
    .setValue('{}')
    .setWidth('150px')
    .setHeight('40px')
    .setFontSize('12px')
    .onChange(function () {
      try {
        ...
        if (JSON.stringify(editor.selected.geometry.userData) != JSON.stringify(userData)) {
          editor.execute(
            new SetGeometryValueCommand(editor, editor.selected, 'userData', userData)
          );
  ...
  geometryUserDataRow.add(
    new UIText(strings.getKey('sidebar/geometry/userdata')).setClass('Label')
  );
  ...
  morphContainer.add(
    new UIText(strings.getKey('sidebar/geometry/morph')).setTextTransform('uppercase')
  );
  ...
  function Morph(index, name, morphTargetInfluences) {
    ...
    const morphInfluence = new UINumber()
      .setWidth('60px')
      .setRange(0, 1)
      .onChange(function updateMorphInfluence() {
