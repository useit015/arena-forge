    ...
    if (geometry) {
      ...
      if (index !== null) {
        containerAttributes.add(
          new UIText(strings.getKey('sidebar/geometry/buffer_geometry/index')).setWidth('70px')
        );
        containerAttributes.add(
          new UIText(editor.utils.formatNumber(index.count)).setFontSize('12px')
        );
      ...
      if (hasMorphTargets === true) {
        ...
        for (const name in morphAttributes) {
          ...
          containerMorphAttributes.add(
            new UIText(editor.utils.formatNumber(morphTargets.length)).setFontSize('12px')
          );
